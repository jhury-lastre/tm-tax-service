import { Client, ClientFilters, ClientResponse, CreateClientRequest, UpdateClientRequest, ClientScenario } from '../types/client.types';
      ...
      const scenarios: ClientScenario[] = await Promise.all(
        clients.map(async (client) => {
          try {
            ...
            // Fetch business data separately (not embedded in clients response)
            let businesses: any[] = [];
            ...
            try {
              ...
              totalBusinessRevenue = businesses.reduce((sum: number, business: any) => {
                const grossSales = parseFloat(business?.grossSales || 0);
                return sum + (isNaN(grossSales) ? 0 : grossSales);
              }, 0);
